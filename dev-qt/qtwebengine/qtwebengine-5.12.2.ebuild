...
PATCHES+=(
	...
	"${FILESDIR}/musl/qt-musl-stackstart-2.patch"
	...
	"${FILESDIR}/musl/qt-musl-remove-cdefs-in-libsync.patch"
...
src_prepare() {
	...
	if use jumbo-build; then
		sed -i -e 's|jumbo_file_merge_limit=[\d]+|jumbo_file_merge_limit=300|' \
			src/core/config/common.pri || die
	fi
