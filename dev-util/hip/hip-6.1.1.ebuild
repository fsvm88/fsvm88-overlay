...
src_install() {
	...
	# files already installed by opencl-headers, which is a runtime dep
	rm "${ED}/usr/include/CL/cl_ext_intel.h" || die
	rm "${ED}/usr/include/CL/cl_gl_ext.h" || die
	rm "${ED}/usr/include/CL/cl_icd.h" || die
	rm "${ED}/usr/include/CL/cl_gl.h" || die
	rm "${ED}/usr/include/CL/opencl.h" || die
	rm "${ED}/usr/include/CL/cl_version.h" || die
	rm "${ED}/usr/include/CL/cl_platform.h" || die
	rm "${ED}/usr/include/CL/cl_dx9_media_sharing_intel.h" || die
	rm "${ED}/usr/include/CL/cl.h" || die
	rm "${ED}/usr/include/CL/cl_va_api_media_sharing_intel.h" || die
	rm "${ED}/usr/include/CL/cl_ext.h" || die
	rm "${ED}/usr/include/CL/cl_egl.h" || die

	# files already installed by opencl-icd-loader, which is a runtime dep
