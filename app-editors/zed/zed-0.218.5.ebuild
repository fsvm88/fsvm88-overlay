...
pkg_setup() {
	...
	# Fix webrtc-sys linking when clang is built with default-libcxx
	# webrtc and other C++ deps are compiled with libstdc++, so we need to explicitly link it
	# The -stdlib flag tells clang which stdlib to use, and -l specifies the library to link
	if tc-is-clang; then
		export CXXFLAGS="${CXXFLAGS} -stdlib=libstdc++"
		# These flags tell the linker to use libstdc++ and link against it
		export RUSTFLAGS="${RUSTFLAGS} -C link-arg=-stdlib=libstdc++ -C link-arg=-lstdc++"
	fi
...
src_prepare() {
	...
	# Fix webrtc-sys C++ stdlib when clang is built with default-libcxx
	# The prebuilt webrtc library uses libstdc++, so we need to ensure
	# the C++ code is compiled with libstdc++ as well
	if tc-is-clang; then
		local webrtc_build_rs="${WORKDIR}/livekit-rust-sdks-5f04705ac3f356350ae31534ffbc476abc9ea83d/webrtc-sys/build.rs"
		sed -i 's/builder\.flag("-std=c++2a");$/builder.flag("-std=c++2a").flag("-stdlib=libstdc++");/' \
			"${webrtc_build_rs}" || die "Failed to patch webrtc-sys build.rs"
		
		# Create cargo config to ensure libstdc++ is linked
		mkdir -p "${S}/.cargo"
		cat > "${S}/.cargo/config.toml" << 'EOF'
[build]
rustflags = ["-C", "link-arg=-stdlib=libstdc++", "-C", "link-arg=-lstdc++"]
EOF
	fi
}
